 ...
 * 动态 jar 类加载器
 */
public class ProxyLoaderJarClassLoader extends URLClassLoader  {
    ...
    /** 构造 */
    public ProxyLoaderJarClassLoader(URL[] urls, ClassLoader parent) {
        super(urls, parent);
        URL url = urls[0];
    ...
    public void initBean(){
        for (Map.Entry<String, Class> entry : cacheClassMap.entrySet()) {
            ...
            if(isSpringBeanClass(cla)){
                ...
                SpringBeanUtils.springBeanUtils.registerBeanDefinition(beanName,beanDefinition);
