...
import com.opencode.weixin.entity.WeChatMessage;
...
public class WeChatRobot {
     ...
     * 微信API配置
     */
    private WeChatBotConfig config;
...
//    private final Map<MsgType, List<Invoke>> mapping = new HashMap<>(8);
    ...
    public WeChatRobot(Builder builder) {
        this.config = builder.config;
        this.session = new LoginSession();
//        this.init();
    }

    public WeChatRobot(WeChatBotConfig config) {
        this(new Builder().config(config));
    }

    public WeChatBotConfig config() {
        return this.config;
    }
...
//    }
    ...
    public static final class Builder {
        ...
        private WeChatBotConfig config = WeChatBotConfig.getInstance();

        public Builder config(WeChatBotConfig config) {
            this.config = config;
            return this;
        }

        public WeChatRobot build() {

            return new WeChatRobot(this);
        }
